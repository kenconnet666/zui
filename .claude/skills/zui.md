...
> - **目录**：无 `components/`、无 `composables/`。内部工具在 `_hooks/`（`useZId`/`usePortal`/`usePopper`/`useRipple`/`useEscapeStack`/`useZVirtualScroll`/`useColorScheme`）与 `_internal/`（`sx`/`input-size`/`color-bridge`/`scrollbar*`/`body-scroll-lock`/`colorScheme`）。**`useStyles`/`useVariants`/`useParts`/`useResponsive` 不存在** —— 组件直接 `computed(() => icss(theme.value, s => {...}))`。
