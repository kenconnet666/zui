import { injectGlobal as emotionInjectGlobal, css } from '@emotion/css'
...
export function injectLayerOrder(layers: readonly string[]): void {
  ...
  emotionInjectGlobal(`@layer ${layers.join(', ')};` as never)
}

/** 把 styles 包进 `@layer <name> { ... }` 注入全局样式表。 */
export function injectLayer(name: string, styles: CSSObject): void {
  emotionInjectGlobal({ [`@layer ${name}`]: styles } as never)
}
