...
<template>
  <section>
    ...
    <ZParagraph>
      ...
      + <strong>iem 等比缩放 size</strong>。启用 ripple 波纹效果, hover / active /
      focus-visible 三态自动接入 Material state layer。
    </ZParagraph>
    ...
    <DemoBlock title="color factory + modifier 链" :source="ColorDemoSource">
      <template #desc>
        <code>:color="(c) =&gt; c._primary"</code> 传入 chain factory,IDE 补全 schema 全部
        color token + 146 CSS 命名色 + modifier 链 (alpha / darken / lighten / mix / shade / tint
        / saturate / desaturate / complement / rotateHue / invert)。用户扩展的 brand
        token(<code>_brandRoyal</code> 等)也自动可用。
      </template>
      <ColorDemo />
    </DemoBlock>
    ...
    <DemoBlock title="number 类型,iem 倍数(默认 1)" :source="SizeDemoSource">
      <template #desc>
        <code>size</code> 是 <strong>iem 倍数</strong>,组件内部按比例算 fontSize / height /
        padding / borderRadius / gap —— 整体比例不变,只是物理像素随 ZBox 字号缩放。
      </template>
      <SizeDemo />
    </DemoBlock>
    ...
    <DemoBlock title="两侧 icon slot" :source="IconDemoSource">
      <template #desc>
        <code>#prefixIcon</code> / <code>#suffixIcon</code> 两个具名 slot 填 ZIcon
        (或任意 inline-flex 内容)。loading 态会临时占据 prefix 位置。
      </template>
      <IconDemo />
    </DemoBlock>
    ...
    <ApiTable
      ...
      :rows="[
        { event: 'click', payload: 'evt: MouseEvent', desc: '按钮点击。disabled=true 或 loading=true 时不触发。' },
      ]"
    />
    ...
    <ApiTable
      ...
      :rows="[
        ...
        { name: 'prefixIcon', desc: '左侧图标区。loading=true 时被加载旋转 icon 占位，此 slot 隐藏。' },
    ...
    <ZParagraph>
      ...
      下表以 <strong>iem = 16px</strong>（固定基准）为例换算各档位的 px 值：
    </ZParagraph>
    <ApiTable
      :columns="[
      ...
      :rows="[
        { size: '0.5',       fontSize: '8px',  height: '16px', paddingY: '4px',  paddingX: '8px',  borderRadius: '3px',  gap: '4px' },
        { size: '0.75',      fontSize: '12px', height: '24px', paddingY: '6px',  paddingX: '12px', borderRadius: '4.5px',gap: '6px' },
        { size: '1（默认）', fontSize: '16px', height: '32px', paddingY: '8px',  paddingX: '16px', borderRadius: '6px',  gap: '8px' },
        { size: '1.25',      fontSize: '20px', height: '40px', paddingY: '10px', paddingX: '20px', borderRadius: '7.5px',gap: '10px' },
        { size: '1.5',       fontSize: '24px', height: '48px', paddingY: '12px', paddingX: '24px', borderRadius: '9px',  gap: '12px' },
