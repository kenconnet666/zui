...
const itemRows = [
  ...
  { name: 'closable', type: 'boolean', default: '—',         desc: '该 Tab 是否可关闭（覆盖全局 closable）。' },
]
...
<template>
  <section>
    ...
    <ZParagraph>
      标签页组件，支持 line / card / segment 三种样式。
      通过 <ZCode code="v-model:value" /> 绑定当前激活的 tab name；
      <ZCode code="closable" /> 和 <ZCode code="addable" /> 用于动态增删 Tab 场景。
    </ZParagraph>
