...
const formPropsRows = [
  { name: 'model',           type: 'Record<string, unknown>', default: '—',        desc: '表单数据对象（reactive，必传）。' },
  { name: 'rules',           type: 'Rules',                   default: '—',        desc: 'async-validator 规则字典（key 为字段名）。' },
  { name: 'labelPlacement',  type: "'top' | 'left'",          default: "'left'",   desc: 'label 位置。' },
  { name: 'validateTrigger', type: "'change'|'blur'|'submit'", default: "'change'", desc: '全局校验触发时机（ZFormItem 可覆盖）。' },
  { name: 'disabled',        type: 'boolean',                 default: 'false',    desc: '整表禁用。' },
  { name: 'css',             type: '(s: Chain) => void',      default: '—',        desc: '根元素 CSS 兜底。' },
]

const formExposeRows = [
  { name: 'validate', type: '() => Promise<void>', desc: '校验所有字段，reject 包含 errors 和 fields。' },
...
const itemPropsRows = [
  ...
  { name: 'rule',            type: 'RuleItem | RuleItem[]',   default: '—',     desc: '单字段规则（优先于 ZForm.rules[prop]）。' },
  { name: 'required',        type: 'boolean',                 default: 'false', desc: '显示必填标记 *（独立于校验规则）。' },
  { name: 'validateTrigger', type: "'change'|'blur'|'submit'", default: '—',    desc: '覆盖 Form 级触发时机。' },
...
<template>
  <section>
    ...
    <ZParagraph>
      基于 <ZCode code="async-validator" /> 的表单容器，<ZCode code="ZFormItem" /> 通过 inject 联动父 Form
      的数据和规则。<ZCode code="labelPlacement" /> 控制标签位置，<ZCode code="validate()" /> expose 方法
      触发全字段校验。
    </ZParagraph>
