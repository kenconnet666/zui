...
const checkAll = computed({
  ...
  set: (v: boolean) => { checked.value = v ? [...all] : [] },
})
...
<template>
  <ZSpace :direction="d => d.column" :size="g => g._middle">
    <ZText :css="s => { s.color._textSecondary; s.fontSize._small }">全选 + indeterminate 半选态</ZText>
    <ZCheckbox v-model:checked="checkAll" :indeterminate="indeterminate" label="全选" />

    <ZText :css="s => { s.color._textSecondary; s.fontSize._small }">CheckboxGroup options 配置</ZText>
    <ZCheckboxGroup v-model:value="checked" :options="fruits" />

    <ZText :css="s => { s.color._textSecondary; s.fontSize._small }">已选:{{ checked.join(', ') || '空' }}</ZText>
  </ZSpace>
</template>
