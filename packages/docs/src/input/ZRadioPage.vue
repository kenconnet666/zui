...
const groupPropsRows = [
  {
    ...
    default: 'null',
    desc: '选中值（v-model:value）。',
  },
  { name: 'disabled', type: 'boolean', default: 'false', desc: '统一禁用所有子项。' },
  { name: 'buttonStyle', type: 'boolean', default: 'false', desc: '子 ZRadio 渲染为按钮组形态。' },
  { name: 'options', type: 'ZRadioOption[]', default: '—', desc: '快捷选项配置。' },
  {
    name: 'direction',
...
const emitsRows = [
  { name: 'update:value', args: 'string|number|boolean', desc: '（ZRadioGroup）选中值变更。' },
  {
    ...
    desc: '（ZRadioGroup）选中变更（与 update:value 等价）。',
  },
]
...
<template>
  <section>
    <ZTitle :level="1">ZRadio / ZRadioGroup 单选框</ZTitle>
    <ZParagraph>
      <ZCode code="ZRadio" /> 需嵌套在 <ZCode code="ZRadioGroup" /> 内使用，Group 通过
      provide/inject 联动。 <ZCode code="buttonStyle" /> 渲染为按钮组形态，<ZCode code="options" />
      支持快捷配置。
    </ZParagraph>
    ...
    <DemoBlock title="buttonStyle / direction / options" :source="ButtonStyleDemoSource">
      <template #desc>
        <ZCode code="buttonStyle" /> 切换为按钮组外观;<ZCode code="direction" /> carrier
        控制排列方向; <ZCode code="options" /> 配置式生成选项。
      </template>
      <ButtonStyleDemo />
    </DemoBlock>
    ...
    <ZTitle :level="2">ZRadioGroup Props</ZTitle>
    <ApiTable
      :columns="[
