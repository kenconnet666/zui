...
<template>
  <section>
    ...
    <ZParagraph>
      文档站「示例 + 源码」卡片，供 docs 站 demo 展示使用。
      核心模式：单个 <ZCode code=".vue" /> 文件同时被作为<strong>运行组件</strong>（进 default slot）
      和<strong>源代码文本</strong>（<ZCode code="?raw" /> import 传 <ZCode code="source" /> prop），
      源码与演示零漂移。
    </ZParagraph>
...
import DemoSource from './demos/MyDemo.vue?raw'`"
  ...
  <Demo />
    ...
    <ZParagraph>
      ...
      任意实例切换主题后全站同步，并持久化到 <ZCode code="localStorage('zui-code-theme')" />。
      支持 22 种 shiki 主题（亮色 / 暗色各 11 种）。
    </ZParagraph>
    ...
    <ApiTable
      ...
      :rows="[
        { name: 'source',           type: 'string',   default: '—（必传）', desc: '源代码文本（通常来自 import ?raw）。' },
        { name: 'title',            type: 'string',   default: '—',        desc: '卡片标题（#header slot 优先级更高）。' },
        { name: 'lang',             type: 'string',   default: `'vue'`,    desc: 'shiki 语言。' },
        { name: 'defaultExpanded',  type: 'boolean',  default: 'false',    desc: '默认是否展开代码。' },
        { name: 'expanded',         type: 'boolean',  default: '—',        desc: '受控展开态（配合 v-model:expanded）。' },
        { name: 'showImports',      type: 'boolean',  default: 'false',    desc: '是否显示源码中的 import 行。' },
        { name: 'copyToastDuration', type: 'number',  default: '1500',     desc: '复制成功 toast 持续毫秒，0 关闭 toast。' },
