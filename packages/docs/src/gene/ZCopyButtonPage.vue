...
const propsRows = [
  ...
  { name: 'size',          type: 'number',                    default: '1',                 desc: 'iem 倍数，所有维度（font/padding/gap/radius）等比缩放。' },
  { name: 'label',         type: 'string',                    default: '—',                 desc: '按钮文字，不传则纯图标模式。' },
  { name: 'copiedLabel',   type: 'string',                    default: "'已复制'",           desc: '复制成功后临时显示的按钮文字。' },
  ...
  { name: 'iconSize',      type: 'number',                    default: 'size * 0.875',      desc: '图标尺寸 iem 倍数，不传随 size 自动缩放。' },
  { name: 'color',         type: '(c: ColorCarrier) => void', default: '_textSecondary',    desc: '文字色 factory，hover 时 _text。' },
...
<template>
  <section>
    ...
    <ZParagraph>
      ...
      点击后按钮文字临时切换为 <ZCode code="copiedLabel" />（默认"已复制"），
      同时可弹顶部 toast 反馈（默认开启）。
      <ZCode code="size" /> 是 iem 倍数，内部所有维度等比缩放。
    </ZParagraph>
    ...
    <ApiTable
      ...
      :rows="[
        { name: 'copy', type: '(success: boolean, text: string) => void', desc: '复制完成（包含成功/失败状态 + 实际复制的文本）。' },
      ]"
    />
  </section>
</template>
