...
const compareRows = [
  ...
  {
    item: '污染 props',
    zellipsis: '无（子元素 props 独立）',
    ztext: '有（椭圆 + 其它 prop 共用）',
  },
...
const propsRows = [
  {
    ...
    type: 'number',
    default: '1',
    desc: '最多显示行数。1 = 单行截断；>1 = 多行截断（-webkit-line-clamp）。',
  },
  ...
  <section>
    ...
    <DemoBlock title="单行 / 多行省略" :source="BasicDemoSource">
      <template #desc>
        ...
        <ZCode code="lines>1" /> 用 <ZCode code="-webkit-line-clamp" />。 需要父容器限定宽度（用
        <ZCode code="css" /> 设 <ZCode code="width" />）。
      </template>
      <BasicDemo />
    </DemoBlock>
    ...
    <DemoBlock title="lines = 1 / 2 / 3" :source="MultiLineDemoSource">
      <template #desc>
        <ZCode code="lines" /> 控制最大显示行数,超出以省略号收束(基于
        <ZCode code="-webkit-line-clamp" />)。
      </template>
      <MultiLineDemo />
    </DemoBlock>
    ...
    <ApiTable
      ...
      :rows="[
        {
          name: 'default',
          desc: '被省略的内容（任意子元素）。lines=1 时单行截断，lines>1 时多行截断。',
        },
      ]"
    />
  </section>
