...
<template>
  <section>
    ...
    <ZParagraph>
      行内文本基础元，提供 <strong>6 维度 carrier factory</strong>（size / weight / color /
      depth / leading / tracking）+ <strong>5 状态布尔</strong>（italic / underline /
      underlineOnHover / strikethrough / mono / ellipsis）+ css 兜底。
      默认渲染 <ZCode code="<span>" />，通过 <ZCode code="tag" /> prop 切换语义元素。
    </ZParagraph>

    <ZTitle :level="2">基础用法</ZTitle>
    <DemoBlock title="尺寸 / 颜色 / 字重" :source="BasicDemoSource">
      <template #desc>
        <ZCode code="size" /> 是 iem 倍数（<ZCode code="number" />）；
        <ZCode code="color" /> / <ZCode code="weight" /> 接 chain carrier factory，
        支持全部 schema token + modifier 链。
      </template>
      <BasicDemo />
    </DemoBlock>

    <ZTitle :level="2">文字状态</ZTitle>
    <DemoBlock title="italic / underline / strikethrough / mono / ellipsis" :source="StateDemoSource">
      <template #desc>
        布尔 prop 零参数驱动；<ZCode code="ellipsis" /> 传 <ZCode code="true" /> = 单行截断，
        传 <ZCode code="number" /> = 多行截断（<ZCode code="-webkit-line-clamp" />）。
      </template>
      <StateDemo />
    </DemoBlock>
    ...
    <ApiTable
      ...
      :rows="[
        ...
        { name: 'weight',         type: '(w: FontWeightCarrier) => void',      default: '—',           desc: '字重 factory。' },
        { name: 'color',          type: '(c: ColorCarrier) => void',           default: 'currentColor',desc: '文字颜色 factory。' },
        { name: 'depth',          type: '(o: OpacityCarrier) => void',         default: '—',           desc: '透明度 factory。' },
        { name: 'leading',        type: '(l: LineHeightCarrier) => void',      default: '—',           desc: '行高 factory。' },
        { name: 'tracking',       type: '(t: LetterSpacingCarrier) => void',   default: '—',           desc: '字距 factory。' },
        ...
        { name: 'ellipsis',       type: 'boolean | number',                    default: 'false',       desc: 'true = 单行截断；number = 多行截断（-webkit-line-clamp）。' },
        { name: 'tag',            type: 'string',                              default: `'span'`,      desc: '根元素 tag，如 em / strong / code / del / ins / mark。' },
