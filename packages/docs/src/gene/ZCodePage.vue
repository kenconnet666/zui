...
<template>
  <section>
    ...
    <ZParagraph>
      ...
      支持<strong>三种模式</strong>：行内 <ZCode code="&lt;code&gt;" />、 块级 fallback
      <ZCode code="&lt;pre&gt;&lt;code&gt;" />、 块级 shiki 高亮（需安装 <ZCode code="shiki" />）。
    </ZParagraph>

    <ZTitle :level="2">三种模式演示</ZTitle>
    <DemoBlock title="inline / block fallback / block highlighted" :source="BasicDemoSource">
      <template #desc>
        行内模式（默认）：<ZCode code="inline=true" />，不需要 lang。 块级高亮：<ZCode
          code="inline=false"
        />
        + <ZCode code="lang" />（需 <ZCode code="pnpm add shiki" />）。 未装 shiki 时自动 fallback
        为纯文本 pre，控制台 warn 一次。
      </template>
      <BasicDemo />
    </DemoBlock>
    ...
    <DemoBlock title="ts / json / bash / css" :source="LangDemoSource">
      <template #desc>
        <ZCode code="lang" /> 接受任意 shiki 支持的语言标识。 装了 shiki 时按语言渲染配色，未装
        shiki 时统一 fallback 为纯文本 pre。
      </template>
      <LangDemo />
    </DemoBlock>
      ...
      :rows="[
        {
          ...
          type: 'string',
          default: '—',
          desc: '代码文本（高亮模式必传，slot 内容无法可靠提取）。',
        },
        {
          name: 'inline',
          type: 'boolean',
          default: 'true',
          desc: 'false = 块级（pre）；true = 行内（code）。',
        },
        {
          name: 'lang',
          type: 'string',
          default: '—',
          desc: 'shiki 语言标识：ts / js / vue / json / bash / css / text 等。',
        },
        {
          name: 'themes',
          type: '{ light: string; dark: string }',
          default: 'light-plus / tokyo-night',
          desc: 'shiki 双主题对。',
        },
        {
          name: 'colorScheme',
          ...
          default: `'auto'`,
          desc: '颜色模式，auto 跟随 OS prefers-color-scheme。',
        },
        {
          name: 'color',
          type: '(c: ColorCarrier) => void',
          default: '—',
          desc: '文字色（fallback / 未高亮时生效；高亮模式由 shiki 控制）。',
        },
    ...
    <ApiTable
      ...
      :rows="[
        {
          name: 'default',
          desc: 'fallback 模式（未启用 shiki 高亮）下的代码内容。高亮模式以 code prop 为准，slot 不参与。',
        },
      ]"
    />
  </section>
