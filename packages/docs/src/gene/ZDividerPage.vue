...
<template>
  <section>
    ...
    <ZParagraph>
      分割线组件，支持<strong>水平 / 垂直</strong>两种方向，水平模式可在中间插入文字。
      支持 <ZCode code="dashed" />（虚线）、<ZCode code="thickness" />（粗细，支持 string 或 iem 倍数 number）、
      <ZCode code="color" /> carrier factory 自定义颜色。
    </ZParagraph>
    ...
    <DemoBlock title="default slot + align" :source="TextDemoSource">
      <template #desc>
        传入 default slot 内容时自动切换为 flex 布局；<ZCode code="align" /> 控制文字偏左/居中/偏右。
        线条与文字同色，通过 <ZCode code="color" /> factory 统一控制。
      </template>
      <TextDemo />
    </DemoBlock>
    ...
    <DemoBlock title="vertical — inline 竖线" :source="VerticalDemoSource">
      <template #desc>
        <ZCode code="vertical=true" /> 渲染为 <ZCode code="inline-block" /> 竖线，需父容器指定高度或用 align-stretch 拉伸。
      </template>
      <VerticalDemo />
    </DemoBlock>
    ...
    <ApiTable
      ...
      :rows="[
        { name: 'vertical',  type: 'boolean',                  default: 'false',    desc: '垂直方向（竖线），需父容器有高度。' },
        { name: 'dashed',    type: 'boolean',                  default: 'false',    desc: '虚线。' },
        { name: 'align',     type: `'left' | 'center' | 'right'`, default: `'center'`, desc: '文字对齐（仅横向 + 有 slot 时生效）。' },
        { name: 'color',     type: '(c: ColorCarrier) => void', default: '_border', desc: '线条与中间文字同色 factory。' },
        { name: 'thickness', type: 'string | number',           default: `'1px'`,   desc: `string → CSS 长度（如 '2px'）；number → iem 倍数（如 0.125 ≈ 2px @ 16px 基准，随 ZBox 缩放）。` },
    ...
    <ApiTable
      ...
      :rows="[
        { name: 'default', desc: '中间文字内容。有内容时自动切换为 flex + 两侧线段布局。' },
      ]"
    />
  </section>
</template>
