...
<template>
  <section>
    ...
    <DemoBlock title="基础 / 禁用项 / 满宽 + 尺寸" :source="BasicDemoSource">
      <template #desc>
        激活项背景 <ZCode code="_bg" /> + 阴影 <ZCode code="_tiny" />； 未激活项 hover
        变色；整体禁用时 <ZCode code="opacity._dim" />。
      </template>
      <BasicDemo />
    </DemoBlock>
      ...
      :rows="[
        {
          ...
          type: 'string | number',
          default: '—',
          desc: '当前选中值（配合 v-model:value）。',
        },
        {
          name: 'options',
          type: 'ZSegmentedOption[]',
          default: '—（必传）',
          desc: '选项列表，每项 { value, label, disabled? }。',
        },
        {
          name: 'size',
          type: 'number',
      ...
      :rows="[
        {
          ...
          desc: 'v-model:value 对应的 emit，选中变化时触发。',
        },
      ]"
    />
  </section>
