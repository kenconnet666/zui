...
<template>
  <section>
    ...
    <DemoBlock title="size / leading / tracking / italic" :source="StyleDemoSource">
      <template #desc>
        carrier factory 调度字号、行高(<ZCode code="leading" />)、字间距(<ZCode
          code="tracking"
        />)等主题 token。
      </template>
      <StyleDemo />
    ...
    <ZParagraph>
      继承 ZText 全部 props（size / weight / color / depth / leading / tracking / italic / underline
      / underlineOnHover / strikethrough / mono / ellipsis / css）， 新增
      <ZCode code="tag" /> 默认值为 <ZCode code="'p'" />。
    </ZParagraph>
    <ApiTable
      :columns="[
      ...
      :rows="[
        {
          ...
          type: 'string',
          default: `'p'`,
          desc: '根元素 tag，如 div / section 等语义元素。',
        },
        {
          name: 'css',
          type: '(s: Chain) => void',
          default: '—',
          desc: '兜底 CSS，如覆盖 marginBottom 自定义间距。',
        },
        {
          name: '...ZText',
          type: '(继承全部 ZText props)',
          default: '—',
          desc: 'size / weight / color / 等。',
        },
      ]"
    />
