...
<template>
  <section>
    ...
    <ZParagraph>
      角标组件，支持<strong>数字 / 字符串 / 红点</strong>三种模式。
      有 default slot 时，徽标绝对定位于子元素右上角；无 slot 时行内显示徽标本体。
      默认颜色 <ZCode code="_danger" />（红色）。
    </ZParagraph>
    ...
    <DemoBlock title="数字 / dot / offset / inline / showZero" :source="BasicDemoSource">
      <template #desc>
        ...
        <ZCode code="dot=true" /> 忽略 value 显示小圆点；
        <ZCode code="showZero=true" /> 让 value=0 也显示。
      </template>
      <BasicDemo />
    </DemoBlock>
    ...
    <ApiTable
      ...
      :rows="[
        { name: 'value',    type: 'number | string', default: '—',       desc: '徽标显示内容（数字或字符串）。' },
        { name: 'dot',      type: 'boolean',         default: 'false',   desc: '红点模式，忽略 value，只显示小圆点。' },
        { name: 'max',      type: 'number',          default: '99',      desc: '数字超过 max 时显示 max+。' },
        { name: 'showZero', type: 'boolean',         default: 'false',   desc: 'value=0 时是否显示徽标。' },
        { name: 'size',     type: 'number',          default: '0.75',    desc: 'iem 倍数（默认 0.75 ≈ 12px @ 16px）。padding / min-width / height / fontSize 等比缩放。' },
