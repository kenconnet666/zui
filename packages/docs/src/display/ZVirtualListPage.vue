...
const propsRows = [
  ...
  { name: 'itemSize',           type: 'number | (i, item) => number', default: '—', desc: '每项高度 iem 倍数（必传）；函数支持动态行高。' },
  { name: 'height',             type: 'number | string',       default: '—',     desc: '容器高度（垂直方向）。iem 倍数或 CSS 字面字符串。' },
  ...
  { name: 'autoMeasure',        type: 'boolean',               default: 'true',  desc: 'ResizeObserver 自动测真实尺寸（动态行高时使用）。' },
  { name: 'scrollEndThreshold', type: 'number',                default: '0',     desc: '触发 scroll-end 的距底阈值（iem 倍数）。' },
...
<template>
  <section>
    ...
    <ZParagraph>
      ...
      <ZCode code="ZList" />、<ZCode code="ZDataTable" />、<ZCode code="ZTree" /> 等大数据组件均基于此实现。
      支持固定行高（快路径）和动态行高（autoMeasure）。
    </ZParagraph>
