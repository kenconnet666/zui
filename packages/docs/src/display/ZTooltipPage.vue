...
const propsRows = [
  ...
  { name: 'placement', type: 'Placement',             default: "'top'",   desc: 'floating-ui placement，如 top / bottom / left / right。' },
  { name: 'trigger',   type: "'hover'|'click'|'focus'|'manual'", default: "'hover'", desc: '触发方式。' },
  { name: 'visible',   type: 'boolean',               default: '—',       desc: '受控显示（manual 模式 / v-model:visible）。' },
...
const slotsRows = [
  ...
  { name: 'content', desc: '自定义 tooltip 内容（覆盖 content prop）。' },
]
...
<template>
  <section>
    ...
    <ZParagraph>
      鼠标悬停时显示简短提示，基于 floating-ui 定位，支持 <ZCode code="hover" /> / <ZCode code="click" /> /
      <ZCode code="focus" /> 多种触发方式，<ZCode code="manual" /> 模式可受控显示。
    </ZParagraph>
