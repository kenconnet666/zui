...
const propsRows = [
  ...
  { name: 'itemSize',    type: 'number | (i) => number', default: '—',     desc: '每项高度 iem 倍数（必传）。' },
  { name: 'height',      type: 'number | string',       default: '—',      desc: '容器高度，iem 倍数或 CSS 字面字符串，虚拟滚动必传。' },
  ...
  { name: 'autoMeasure', type: 'boolean',               default: 'true',   desc: 'ResizeObserver 自动测真实尺寸。' },
...
<template>
  <section>
    ...
    <ZParagraph>
      基于虚拟滚动的数据列表，支持头尾区域、边框、空态、动态行高。
      通过 <ZCode code="itemSize" /> 设置每项高度（iem 倍数），<ZCode code="height" /> 设置容器高度（必传）。
    </ZParagraph>
