...
const propsRows = [
  ...
  { name: 'selected',  type: '(string|number)[]',     default: '[]',      desc: '选中行 key 数组（v-model:selected）。' },
  { name: 'sort',      type: 'ZDataTableSort | null', default: 'null',    desc: '排序状态（v-model:sort）。' },
...
const columnRows = [
  ...
  { name: 'width',     type: 'number | string',   default: '—',      desc: '列宽（iem 倍数或 CSS 字面字符串）；不传则 flex 均分。' },
  { name: 'minWidth',  type: 'number',            default: '5',      desc: 'flex 均分时的最小列宽（iem）。' },
  { name: 'align',     type: "'left'|'center'|'right'", default: "'left'", desc: '对齐方式。' },
  { name: 'accessor',  type: '(row) => unknown',  default: '—',      desc: '取单元格值（默认 row[key]）。' },
  { name: 'render',    type: '(row, idx) => VNode | string', default: '—', desc: '自定义渲染，优先级高于 accessor。' },
  { name: 'sortable',  type: 'boolean | (a, b) => number', default: 'false', desc: 'true 用默认比较；函数走自定义。' },
]
