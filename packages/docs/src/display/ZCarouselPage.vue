...
const propsRows = [
  { name: 'items',      type: 'T[]',                  default: '—',     desc: '轮播数据数组，通过 #default slot scope { item, index } 渲染。' },
...
const emitsRows = [
  { name: 'update:current', args: 'number', desc: 'current index 变更。' },
  { name: 'change',         args: 'number', desc: '轮播切换后触发。' },
]
</script>
...
<template>
  <section>
    ...
    <ZParagraph>
      滑动卡片轮播，支持自动播放、指示点、箭头按钮。通过 <ZCode code="#default" /> slot scope 渲染每张幻灯片。
    </ZParagraph>
