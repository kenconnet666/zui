...
const propsRows = [
  ...
  { name: 'fontSize', type: 'number', default: '14', desc: '字号（px，Canvas 绘制单位）。' },
  {
    name: 'color',
    type: 'string',
    default: '主题 _text 10%',
  ...
  { name: 'rotate', type: 'number', default: '-22', desc: '旋转角度（deg）。' },
  {
    ...
    type: '(s: Chain) => void',
    default: '—',
    desc: '容器根元素 CSS 兜底（通常用于设置 height）。',
  },
]

const slotsRows = [{ name: 'default', desc: '被水印覆盖的内容区域。' }]
</script>

<template>
  <section>
    <ZTitle :level="1">ZWatermark 水印</ZTitle>
    <ZParagraph>
      通过 Canvas 生成水印图案并平铺作为背景，覆盖在 slot 内容之上。 水印层设置
      <ZCode code="pointer-events: none" />，不影响用户操作。
      支持文字、字号、颜色、旋转角度和间距自定义。
    </ZParagraph>

    <ZTitle :level="2">基础用法</ZTitle>
    <DemoBlock title="基础水印 / 自定义样式" :source="BasicDemoSource">
      <template #desc>
        <ZCode code="content" /> 为水印文字（必传）； <ZCode code="fontSize" /> 单位 px（Canvas
        绘制）； <ZCode code="rotate" /> 单位 deg（默认 -22）。
      </template>
      <BasicDemo />
    </DemoBlock>
    ...
    <DemoBlock title="rotate / gap / color / fontSize" :source="RotateDemoSource">
      <template #desc>
        <ZCode code="rotate" /> 控制角度;<ZCode code="gap" /> 控制平铺密度;<ZCode code="color" /> 用
        rgba 控制透明度。
      </template>
      <RotateDemo />
    </DemoBlock>
