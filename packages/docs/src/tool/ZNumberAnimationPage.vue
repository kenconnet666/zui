...
const propsRows = [
  ...
  { name: 'duration', type: 'number', default: '1500', desc: '动画时长（ms）。' },
  { name: 'precision', type: 'number', default: '—', desc: '小数位数。不传则根据目标值自动决定。' },
  { name: 'separator', type: 'string', default: "','", desc: '千分位分隔符，设为空字符串可关闭。' },
...
<template>
  <section>
    <ZTitle :level="1">ZNumberAnimation 数字动画</ZTitle>
    <ZParagraph>
      数字滚动动画组件，从 <ZCode code="from" /> 平滑过渡到 <ZCode code="to" />， 使用 easeOutCubic
      缓动函数。支持千分位分隔符和小数精度。 目标值变化时自动重新触发动画。
    </ZParagraph>

    <ZTitle :level="2">基础用法</ZTitle>
    <DemoBlock title="基础 / 小数 / 无分隔符 / 动态目标值" :source="BasicDemoSource">
      <template #desc>
        <ZCode code="to" /> 变化时动画自动重新触发； <ZCode code="precision" /> 控制小数位数；
    ...
    <DemoBlock title="precision / separator / 动态 to" :source="PrecisionDemoSource">
      <template #desc>
        <ZCode code="precision" /> 控制小数位;<ZCode code="separator" /> 千分位分隔符; 改变
        <ZCode code="to" /> 自动重新播放动画。
      </template>
      <PrecisionDemo />
    </DemoBlock>
    ...
    <ApiTable
      :columns="[
        ...
        { key: 'type', label: '类型', mono: true, width: '140px' },
