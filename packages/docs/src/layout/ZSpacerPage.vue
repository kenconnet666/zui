...
<template>
  <section>
    ...
    <ZParagraph>
      ...
      <strong>必须放在 flex 容器内</strong>（<ZCode code="ZFlex" /> / <ZCode code="ZSpace" /> / 任何
      <ZCode code="display: flex" /> 父级）， 在 grid / block 容器内无效果。
    </ZParagraph>

    <ZTitle :level="2">基础用法</ZTitle>
    <DemoBlock title="工具栏 push-apart / grow 比例" :source="BasicDemoSource">
      <template #desc>
        工具栏场景：左侧标题 + <ZCode code="&lt;ZSpacer /&gt;" /> + 右侧操作组， ZSpacer
        自动填满剩余空间。
      </template>
      <BasicDemo />
    </DemoBlock>
    ...
    <DemoBlock title="grow=0 + basis：固定占位,两侧自适应" :source="BasisDemoSource">
      <template #desc>
        <ZCode code="basis" /> 设 <ZCode code="flex-basis" />;配合
        <ZCode code=":grow='0'" /> 即得固定宽度占位块。
      </template>
      <BasisDemo />
    </DemoBlock>
    ...
    <ApiTable
      ...
      :rows="[
        ...
        {
          name: 'basis',
          type: '(b: flexBasis carrier) => void',
          default: 'auto',
          desc: 'flex-basis carrier factory，默认 auto。',
        },
