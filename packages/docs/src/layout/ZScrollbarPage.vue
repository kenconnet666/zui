...
<template>
  <section>
    ...
    <ZParagraph>
      <strong>使用要求</strong>：父级或 <ZCode code=":css" /> prop 必须给根元素提供明确高度 （<ZCode
        code="height: 100%"
      />
      / <ZCode code="flex-grow: 1; min-height: 0" /> 等）， 否则内容区为 0 高。也可用
      <ZCode code="maxHeight" /> prop 直接设置最大高度。
    </ZParagraph>
    ...
    <DemoBlock title="maxHeight / 外部 css 设高度" :source="BasicDemoSource">
      <template #desc>
        悬停容器可看到自定义滚动条淡入效果。
        <ZCode code="maxHeight" /> 模式：超出最大高度后才滚动； 外部
    ...
    <ApiTable
      ...
      :rows="[
        ...
        },
        {
          ...
          type: '(s: Chain) => void',
          default: '—',
          desc: '根元素 CSS 兜底（常用于设固定 height / flex-grow）。',
        },
      ]"
    />
    ...
    <ApiTable
      ...
      :rows="[
        {
          name: 'default',
          desc: '可滚动内容，渲染到内层 scroller div 内。超出容器高度时显示自定义滚动条。',
        },
      ]"
    />
      ...
      :rows="[
        {
          ...
          desc: '内层可滚动 div 的 ref，可用于编程式滚动（scrollTop = 0 等）。',
        },
