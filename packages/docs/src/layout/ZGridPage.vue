...
<template>
  <section>
    ...
    <ZParagraph>
      CSS Grid 布局容器。<ZCode code="cols" /> / <ZCode code="rows" /> 支持三种形式：
      数字（<ZCode code=":cols='3'" /> → <ZCode code="repeat(3, minmax(0, 1fr))" />）、
      字符串（直接作 grid-template）、响应式对象（按断点自动注入 <ZCode code="@media" />）。
    </ZParagraph>
    ...
    <ApiTable
      ...
      :rows="[
        { name: 'cols',         type: 'number | string | Partial<Record<breakpoint, number|string>>', default: '—', desc: '列定义。数字 → repeat(N, 1fr)；字符串 → 直接作 grid-template-columns；对象 → 响应式断点（tiny/small/middle/large/huge）。' },
        { name: 'rows',         type: '同 cols',                         default: '—',     desc: '行定义。同 cols，作用于 grid-template-rows。默认不设（自动行）。' },
        { name: 'gap',          type: '(g: gap carrier) => void',        default: '—',     desc: 'gap carrier factory。' },
        { name: 'justifyItems', type: '(c: justifyItems carrier) => void', default: '—',  desc: 'justify-items carrier factory。' },
        { name: 'alignItems',   type: '(c: alignItems carrier) => void', default: '—',    desc: 'align-items carrier factory。' },
        { name: 'inline',       type: 'boolean',                         default: 'false', desc: 'true → display: inline-grid。' },
    ...
    <ApiTable
      ...
      :rows="[
        { name: 'default', desc: 'grid 容器内的子元素，按 cols / rows 自动布局。' },
      ]"
    />
  </section>
</template>
