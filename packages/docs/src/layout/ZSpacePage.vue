...
<template>
  <section>
    ...
    <ZParagraph>
      等间距布局（类 antd Space）。与 <ZCode code="ZFlex" /> 的区别： <ZCode code="ZSpace" /> 默认
      <ZCode code="align: center" /> + <ZCode code="gap: _small" />，
    ...
    <DemoBlock title="默认间距 / 自定义 gap / 垂直方向 / wrap" :source="BasicDemoSource">
      <template #desc>
        ...
        <ZCode code="direction" /> 切换横/纵排； <ZCode code="wrap" /> 控制换行。
      </template>
      <BasicDemo />
    </DemoBlock>
    ...
    <DemoBlock title="align: center（默认） vs flex-start" :source="AlignDemoSource">
      <template #desc>
        子项高度不一致时,<ZCode code="align" /> 决定它们在交叉轴的对齐;默认
        <ZCode code="center" />。
      </template>
      <AlignDemo />
    </DemoBlock>
      ...
      :rows="[
        {
          ...
          default: '—',
          desc: '排列方向 carrier factory，默认不写 = row。',
        },
        {
          name: 'size',
          type: '(g: gap carrier) => void',
          default: '_small',
          desc: '子元素间距（gap carrier factory），默认 _small。',
        },
        {
          name: 'align',
          type: '(c: alignItems carrier) => void',
          default: 'center',
          desc: '交叉轴对齐 carrier factory，默认 center。',
        },
        {
          name: 'wrap',
          type: '(c: flexWrap carrier) => void',
          default: '—',
          desc: '换行 carrier factory，默认不写 = nowrap。',
        },
        {
          name: 'inline',
          type: 'boolean',
    ...
    <ApiTable
      ...
      :rows="[{ name: 'default', desc: '等间距排列的子元素列表。' }]"
    />
  </section>
</template>
