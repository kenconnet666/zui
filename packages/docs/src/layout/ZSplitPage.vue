...
<template>
  <section>
    ...
    <ZParagraph>
      可拖拽的两栏布局，支持水平/垂直分割。
      <ZCode code="v-model:ratio" /> 控制第一栏占比（0~1，默认 0.5），
      <ZCode code="min" /> / <ZCode code="max" /> 限制拖拽范围。
    </ZParagraph>

    <ZTitle :level="2">基础用法</ZTitle>
    <DemoBlock title="水平分割 / 垂直分割（v-model:ratio）" :source="BasicDemoSource">
      <template #desc>
        拖动分割线即可调整比例；<ZCode code="ratio" /> 故意不叫 <ZCode code="size" />
    ...
    <ApiTable
      ...
      :rows="[
        { name: 'direction', type: `'horizontal' | 'vertical'`, default: `'horizontal'`, desc: '分割方向：horizontal 左右，vertical 上下。' },
        { name: 'ratio',     type: 'number',                    default: '0.5',          desc: '第一栏占容器比例（0~1），配合 v-model:ratio。' },
        ...
        { name: 'css',       type: '(s: Chain) => void',        default: '—',            desc: '根元素 CSS 兜底（通常用于设 height）。' },
      ]"
    />
    ...
    <ApiTable
      ...
      :rows="[
        { name: 'update:ratio', type: '(value: number) => void', desc: 'v-model:ratio 对应的 emit，拖拽时持续触发。' },
      ]"
    />
  </section>
</template>
