...
const apiRows = [
  ...
  {
    name: 'loading',
    type: '(content: string, duration?: number) => id',
    desc: '加载中消息，默认不自动关闭（duration=0）。',
  },
...
const propsRows = [
  {
    ...
    type: 'ZMessageItem[]',
    default: '—（必传）',
    desc: 'message 数组，业务方自管 state。',
  },
  {
    name: 'css',
    type: '(s: Chain) => void',
    default: '—',
    desc: '容器 CSS 兜底（fixed 吸顶居中）。',
  },
]
...
const itemRows = [
  ...
  { name: 'content', type: 'string', default: '—（必传）', desc: '消息文本内容。' },
  {
    ...
    type: 'number',
    default: '3000',
    desc: '持续时间 ms，0 = 不自动关闭（loading 默认 0）。',
  },
...
const emitsRows = [
  {
    ...
    desc: '某条消息到时关闭时触发，参数为该条 message 的 id。',
  },
]
</script>
...
<template>
  <section>
    ...
    <ZParagraph>
      顶部居中 Toast 消息，通过 <ZCode code="Teleport" /> 渲染到 <ZCode code="body" />， 支持 info /
      success / warning / error / loading 五种语义。 推荐使用
      <ZCode code="createMessageApi()" /> 工厂模式，在应用任意位置调用，无需手动管理组件状态。
    </ZParagraph>

    <ZTitle :level="2">基础用法</ZTitle>
    <DemoBlock title="五种语义 / loading → success" :source="BasicDemoSource">
      <template #desc>
        <ZCode code="createMessageApi()" /> 返回工厂对象； <ZCode code="msg.loading()" /> 返回
        id，调 <ZCode code="msg.close(id)" /> 可手动关闭。
      </template>
      <BasicDemo />
    </DemoBlock>
