...
function finish() {
  if (timer) clearInterval(timer)
  progress.value = 100
  setTimeout(() => { progress.value = null }, 500)
}
...
function error() {
  ...
  isError.value = true
  progress.value = 60
  setTimeout(() => { progress.value = null; isError.value = false }, 1500)
}
</script>
...
<template>
  <ZFlex :direction="d => d.column" :gap="g => g._large">
    ...
    <ZFlex :direction="d => d.column" :gap="g => g._small">
      ...
      <ZLoadingBar :value="30" :css="s => { s.position.relative; s.borderRadius._small }" />
      <ZLoadingBar :value="70" :css="s => { s.position.relative; s.borderRadius._small }" />
      <ZLoadingBar :value="50" :error="true" :css="s => { s.position.relative; s.borderRadius._small }" />
    </ZFlex>
  </ZFlex>
</template>
