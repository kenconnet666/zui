...
const apiRows = [
  {
    name: 'info',
    type: '(title: string, desc?: string, duration?: number) => id',
    desc: '普通通知。',
  },
  {
    name: 'success',
...
<template>
  <section>
    ...
    <ZParagraph>
      角落弹出通知，比 ZMessage 功能更丰富：支持 <ZCode code="title" /> +
      <ZCode code="description" /> 两行内容、 可关闭按钮、自定义位置（右上角 / 左上角 / 右下角 /
      左下角）。 推荐使用 <ZCode code="createNotificationApi()" /> 工厂模式。
    </ZParagraph>

    <ZTitle :level="2">基础用法</ZTitle>
    <DemoBlock title="五种语义 / 无描述 / loading → success" :source="BasicDemoSource">
      <template #desc>
        <ZCode code="createNotificationApi()" /> 返回工厂对象； 通知默认 4500ms 后自动关闭； loading
        通知默认不自动关闭，需手动 <ZCode code="close(id)" />。
      </template>
      <BasicDemo />
    </DemoBlock>
